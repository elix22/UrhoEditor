...
std::string GetAppDataPath(const std::string& file)
{
    ...
    return QString("%1/%2").arg(basePath.c_str(), file.c_str()).toStdString();
}
...
std::string GetDocumentsPath(const std::string& file)
{
    ...
    return QString("%1/%2").arg(docPath.c_str(), file.c_str()).toStdString();
}
...
{
    QSexyToolTip* helpWindow_ = new QSexyToolTip(UrhoEditor::GetInstance());
