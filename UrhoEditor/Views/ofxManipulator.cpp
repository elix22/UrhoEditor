...
void ofxManipulator::mouseDragged(int x, int y, float deltaX, float deltaY, int button)
{
    ...
    case (MANIPULATOR_TRANSLATION) :
    {
        ...
        else 
        {
            ...
            case TRANSLATION_XY: df = Urho3D::Vector3(df.DotProduct(axe_x), df.DotProduct(axe_y), 0); break;
            case TRANSLATION_XZ: df = Urho3D::Vector3(df.DotProduct(axe_x), 0, df.DotProduct(axe_z)); break;
            case TRANSLATION_YZ: df = Urho3D::Vector3(0, df.DotProduct(axe_y), df.DotProduct(axe_z)); break;
            }
