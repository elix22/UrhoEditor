...
namespace SprueEditor
{
    ...
    void UrhoRaySelectController::Update()
    {
        if (scene_)
        {
            ...
            auto current = viewManager_->GetRenderWidget()->GetSelectron()->GetSelected<UrhoUI::SerializableDataSource>();
            ...
            if (newSelection != selected.end())
                viewManager_->GetRenderWidget()->GetSelectron()->SetSelected(0x0, std::shared_ptr<DataSource>(new UrhoUI::NodeDataSource(newSelection->node_)));
            else
