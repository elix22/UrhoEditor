...
void CreateGrid(Scene* scene, int gridSize, const Vector3& gridScale, bool is2D)
{
    ...
    for (unsigned i = 0; i <= size; ++i)
    {
        ...
        bool lineSubdiv = fmodf(i, subdivisionSize) == 0.0f;
    ...
    // Draw X axis forward
    grid->DefineVertex(Vector3(halfSizeScaled, 0, 0));
    grid->DefineColor(gridXColor);
    grid->DefineVertex(Vector3(quarterSize, 0, eightSize));
    grid->DefineColor(gridXColor);
    ...
    // Draw Z axis forward
    grid->DefineVertex(Vector3(0, 0, halfSizeScaled));
    grid->DefineColor(gridZColor);
    grid->DefineVertex(Vector3(eightSize, 0, quarterSize));
    grid->DefineColor(gridZColor);
