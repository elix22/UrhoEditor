...
void PerspectiveView::RenderUpdate()
{
    ...
    if (camPos_)
        camPos_->SetText(QString("X: %1 Y: %2 Z: %3").arg(cameraNode_->GetWorldPosition().x_, cameraNode_->GetWorldPosition().y_, cameraNode_->GetWorldPosition().z_).toStdString().c_str());
    if (triStats_)
