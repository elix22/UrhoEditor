...
namespace SprueEditor
{
    ...
    void EditMenuBuilder::Build(UrhoEditor* editor)
    {
        ...
        RegisteredAction* cutAct = new RegisteredAction("Edit", "Cut");
        cutAct->setIcon(QIcon(":/Images/shared/Cut.png"));
        cutAct->SetShortcut(QKeySequence::Cut);
        RegisteredAction* copyAct = new RegisteredAction("Edit", "Copy");
        copyAct->setIcon(QIcon(":/Images/shared/Copy.png"));
        copyAct->SetShortcut(QKeySequence::Copy);
        RegisteredAction* pasteAct = new RegisteredAction("Edit", "Paste");
        ...
        connect(cutAct, &QAction::triggered, [=](bool state) {
            if (editor->focusWidget() && editor->focusWidget()->metaObject()->indexOfMethod("cut()") >= 0)
        ...
        connect(copyAct, &QAction::triggered, [=](bool state) {
            ...
            {
                if (editor->focusWidget() && editor->focusWidget()->metaObject()->indexOfMethod("copy()") >= 0)
        ...
        connect(pasteAct, &QAction::triggered, [=](bool state) {
            if (editor->focusWidget() && editor->focusWidget()->metaObject()->indexOfMethod("paste()") >= 0)
