...
namespace SprueEditor
{
    ...
    UrhoEditor* UrhoEditor::instance_ = 0x0;

    UrhoEditor::UrhoEditor(Settings* settings, const std::vector<GuiBuilder*>& guiBuilders, const std::vector<DocumentHandler*>& docHandlers, QWidget *parent) :
        QMainWindow(parent),
        settings_(settings),
        renderWidget_(0x0),
        ...
        secondaryTaskProcessor_(new TaskProcessor(true))
    {
        ...
        connect(ApplicationCore::GetDocumentManager(), &DocumentManager::ActiveDocumentChanged, this, &UrhoEditor::ActiveDocumentChanged);
        connect(ApplicationCore::GetDocumentManager(), &DocumentManager::DocumentClosed, this, &UrhoEditor::DocumentClosed);
        ...
        connect((SearchLineEdit*)searchText_, &SearchLineEdit::SearchPerformed, this, &UrhoEditor::SearchPerformed);
        connect(searchBtn, &QPushButton::clicked, this, &UrhoEditor::SearchPerformed);
        ...
        ribbon_->setFixedHeight(ribbon_->tabBar()->height() + (96 + 24));
        ...
        // App menu
        {
            ...
            connect(UrhoEditor::GetInstance(), &UrhoEditor::WindowMoved, appMenu_, &QSexyToolTip::reposition);
            ...
            menu->AddExpansionButton(QIcon(":/Images/godot/icon_new.png"), "New", new SprueEditor::NewAppPage());
            menu->AddExpansionButton(QIcon(":/Images/godot/icon_open.png"), "Open", new SprueEditor::OpenAppPage());
            menu->AddSeperator();
        ...
        connect(taskProcessor_, &TaskProcessor::TaskChanged, this, &UrhoEditor::SetTask, Qt::ConnectionType::QueuedConnection);
    }

    UrhoEditor::~UrhoEditor()
    {
        RegisteredAction::Save();
    }

    UrhoEditor* UrhoEditor::GetInstance()
    {
        return instance_;
    }

    ViewManager* UrhoEditor::GetViewManager() const { return renderWidget_->GetViewManager(); }

    void UrhoEditor::CreateWidgets()
    {
        ...
        renderWidget_->SetSelectron(&objectSelectron_);
    ...
    QMenu* UrhoEditor::GetOrCreateMenu(const std::string& name, const std::string& title)
    {
    ...
    QToolBar* UrhoEditor::GetOrCreateToolBar(const std::string& name, const std::string& title)
    {
    ...
    LocalizedDock* UrhoEditor::GetOrCreateDock(const std::string& name, const std::string& title, const std::string& defaultAccelerator)
    {
        ...
        if (found == docks_.end())
        {
            ...
            connect(newDock, &QDockWidget::dockLocationChanged, this, &UrhoEditor::WindowMoved);
    ...
    LocalizedDock* UrhoEditor::GetDock(const std::string& name)
    {
        auto found = docks_.find(name);
        if (found != docks_.end())
            return found->second;
    ...
    void UrhoEditor::AddDock(const std::string& name, LocalizedDock* dock)
    {
        auto found = docks_.find(name);
        if (found != docks_.end())
            return;
        dock->setObjectName(name.c_str());
        connect(dock, &QDockWidget::dockLocationChanged, this, &UrhoEditor::WindowMoved);
        docks_[name] = dock;
    ...
    void UrhoEditor::closeEvent(QCloseEvent* event)
    {
    ...
    void UrhoEditor::moveEvent(QMoveEvent* event)
    {
    ...
    void UrhoEditor::CreateMenuBar()
    {
        menuBar()->addMenu(windowMenu_);
    }

    void UrhoEditor::CreateStatusBar()
    {
        ...
        taskLbl_ = new QLabel("Task");
        statusBar()->addPermanentWidget(taskLbl_);
    }

    void UrhoEditor::HandleBackgroundAction()
    {
        // TODO 
    ...
    void UrhoEditor::HandleTaskTimeout()
    {
        taskProcessor_->Update();
    }

    void UrhoEditor::ActiveDocumentChanged(DocumentBase* newDoc, DocumentBase* oldDoc)
    {
    ...
    void UrhoEditor::DocumentClosed(DocumentBase* closingDoc)
    {
        if (closingDoc && closingDoc->GetToolshelfWidget())
            viewBar_->removeWidget(closingDoc->GetToolshelfWidget());
    }

    void UrhoEditor::SetTask(QString lbl)
    {
        taskLbl_->setText(lbl);
    }

    void UrhoEditor::SaveLayout(const std::string& name, const std::string& intoFile)
    {
        std::string filePath;
        if (intoFile.empty())
        {
        ...
        {
            filePath = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation).toStdString();
            filePath += QString("/%1").arg(intoFile.c_str()).toStdString();
        }

        QFile file(filePath.c_str());
        if (!file.open(QIODevice::WriteOnly))
        {
            LOGERROR(QString("Unable to open layout file for write: %1").arg(filePath.c_str()));
            return;
        }
    ...
    void UrhoEditor::RestoreLayout(const std::string& fromFile)
    {
        LOGINFO(QString("Loading layout: %1").arg(fromFile.c_str()));
        std::string filePath;
        if (fromFile.empty())
        {
            filePath = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation).toStdString();
        ...
        else
        {
            ...
            {
                filePath = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation).toStdString();
                filePath += QString("/%1").arg(fromFile.c_str()).toStdString();
            }
        }

        QFile file(filePath.c_str());
        LOGINFO(QString("Mapped layout to load as: %1").arg(filePath.c_str()));
        if (!file.open(QIODevice::ReadOnly))
        {
            LOGERROR(QString("Unable to open layout file for read: %1").arg(filePath.c_str()));
            return;
        }
    ...
    std::vector< std::pair<std::string, std::string> > UrhoEditor::EnumerateLayouts() const
    {
    ...
    void UrhoEditor::SearchPerformed()
    {
        ...
        if (!txt.isEmpty())
        {
            ...
            statusBar()->showMessage(QString("Found %1 search %?1=2?result:results;.").arg((unsigned)searchResults.size(), 1));
