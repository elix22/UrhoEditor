...
namespace UrhoUI
{
    ...
    void UrhoSceneView::SelectionChanged(Selectron* newSelection)
    {
        ...
        {
            if (auto object = newSelection->GetSelected<UrhoUI::SerializableDataSource>(i))
            {
                ...
                if (selectedNode)
                {
                    std::shared_ptr<UrhoUI::UrhoGizmo> giz(new UrhoUI::UrhoGizmo(selectedNode));
                    giz->Construct();
    ...
    void UrhoSceneView::RenderUpdate()
    {
        ...
        {
            if (auto selected = sel->GetSelected<UrhoUI::SerializableDataSource>(i))
            {
        ...
        camPos_->SetText(QString("X: %1 Y: %2 Z: %3").arg(cameraNode_->GetWorldPosition().x_, cameraNode_->GetWorldPosition().z_, cameraNode_->GetWorldPosition().y_).toStdString().c_str());
