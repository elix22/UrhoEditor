...
#define URHO_PROPERTY_UNDO(EDITING, PROPERTY, NEWVALUE) { SmartCommand* cmd = new UrhoUI::UrhoPropertyCmd(EDITING, PROPERTY, NEWVALUE, EDITING->GetObject()->GetAttribute(name_)); \
        ...
        if (auto data = std::dynamic_pointer_cast<SerializableDataSource>(datasource))
            if (auto editing = data->GetObject())
            {
                if (auto property = Attribute(editing.Get()))
                {
                    ...
                    //SprueEditor::UrhoEditor::GetInstance()->GetDocumentManager()->GetActiveDocument()->GetUndoStack()->push(cmd);
