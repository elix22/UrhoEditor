...
void DataGridWidget::SelectionChanged(void* source, Selectron* sel)
{
    ...
    for (unsigned i = 0; i < sel->GetSelectedCount(); ++i)
    {
        ...
        if (auto handler = GetHandlerFor(object))
        {
            dataSources_.push_back(object);
            auto names = handler->GetColumns(object);
            for (auto name : names)
            {
                if (i == 0)
                {
    ...
    {
        {
            ...
            {
                std::string column = appearanceOrder_[col];
                if (filtered_.find(column) == filtered_.end())
                    continue;
                table_->setHorizontalHeaderItem(c, new QTableWidgetItem(column.c_str()));
                ++c;
            }
        }
        ...
        for (unsigned r = 0; r < dataSources_.size(); ++r)
        {
            ...
            if (auto handler = GetHandlerFor(object))
            {
                ...
                for (unsigned col = 0; col < appearanceOrder_.size(); ++col)
                {
                    ...
                    {
                        QTableWidgetItem* item = new QTableWidgetItem(handler->ToText(object, column));
                        if (handler->CanSetText(object, column))
    ...
    if (auto object = dataSources_[item->row()])
    {
        if (auto handler = GetHandlerFor(object))
        {
            ...
            for (unsigned col = 0; col < appearanceOrder_.size(); ++col)
            {
                ...
                if (c == item->column())
                {
                    handler->FromText(object, column, item->text().toStdString());
                    break;
                }
                ++c;
            }
...
DataGridWidget::DataSourceHandler* DataGridWidget::GetHandlerFor(std::shared_ptr<DataSource> dataSource)
{
    for (auto item : handlers_)
        if (item->CanHandleDataSource(dataSource))
            return item;
    return 0x0;
}